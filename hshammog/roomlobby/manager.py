...
def run(server_mode):
    ...
    if server_mode == 'mq':
        ...
        from roomlobby.gateway import Gateway
        ...
        RoomServer(cfg.mq_servers[0], cfg.mq_outport,
                   cfg.mq_inport, cfg.zk_servers[0], cfg.zk_path).run()
    elif server_mode == 'monitor':
        from roomlobby.monitor import Monitor
        Monitor(cfg.zk_servers[0], cfg.zk_path).run()
